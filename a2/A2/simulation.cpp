...
void Simulation::Init(Scene* scene)
{
    ...
    for (uint32_t loadedMeshID : loadedMeshIDs)
    {
		...
		mSpinningTransformID = newTransformID;

		scene->Transforms[newTransformID].Scale = glm::vec3(1.0f);
    }
    ...
    for (uint32_t loadedMeshID : loadedMeshIDs)
    {
        // place a teapot on top of the cube
        {
			...
			scene->Transforms[scene->Instances[newInstanceID].TransformID].ParentID = mSpinningTransformID;
        }
